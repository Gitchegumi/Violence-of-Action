...
- [X] T015 Implement `scenes/ui/radial_unit_info_panel.tscn` (root `Control` named `RadialUnitInfoPanel`, hidden default, child nodes: TextureRect, Name Label, Stats VBoxContainer, Abilities RichTextLabel). *(Later removed: deploy-hover now reuses the shared `UnitInfoPanel` instead of a separate radial overlay.)*
- [X] T016 Implement `scripts/ui/radial_unit_info_panel.gd` with `show_unit(dict)` + `hide_panel()` fulfilling existing test expectations (visibility toggles only initially). *(Later removed alongside T015.)*
